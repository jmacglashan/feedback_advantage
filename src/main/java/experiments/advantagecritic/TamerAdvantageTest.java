...
public class TamerAdvantageTest {
    ...
    public static void main(String[] args) {
        ...
        GDHumanRewardModel hrf = new GDHumanRewardModel(vfa, 0.1);
        ...
        AdvantageFeedback feedback = new AdvantageFeedback(tamer, (EnumerablePolicy)tamerPolicy, domain, new SimpleHashableStateFactory(), 0.99, 0.01, 100);
        env.addObservers(feedback); //add our artificial human feedback as an observe to the environment

        List<Episode> episodes = new ArrayList<>();
        for(int i = 0; i < 2000; i++){
            Episode e = tamer.runLearningEpisode(env, 200);
            episodes.add(e);
